"""
Class containing info from an activity completion table
...
def read_ac_csvs(acti_file, usr_file=None):
    """
    Read csv files to generate a ActivityCompletion object

    Parameters
    ----------
    acti_file : str
        Name of csv file containing the activity completion
    usr_file : str
        Name of csv file containing users. Optional: if it's not included, cdat.users = None

    """
    if usr_file:
        adat = ActivityCompletion(pd.read_csv(acti_file),pd.read_csv(usr_file))
    else:
        adat = ActivityCompletion(pd.read_csv(acti_file), None)
    return adat

class ActivityCompletion(object):
    """
    Contains and processes data using an activity completion file and a user database
